...
void CloseExcessWindows() // test specific, ensure a clean state; can be made better with RPF 12+
{
	...
	GlobalizeQueryId(szGlobalQuery, 1024, COMPUTER_CLOSE);
...
//    
//    VerifyTimewarp_StartMenuEntryPoint 
//  
STDAPI Timewarp_EntryPoint(LPVOID pvReserved)
{
    BOOL    fPassed = FALSE;
	...
	//LogTestInfoW(__FUNCTIONW__, L"String Part [%s]",wcsstr(pwszTemplate, L"%2!c!"));
	...
	LogTestBeginW(__FUNCTIONW__, L"Verify Timewarp available on Local drive");    
        ...
        if (SUCCEEDED(hr))  
        {
			...
			if (SUCCEEDED(hr))  
			{
				LOG_FAILED_HR(RightButtonClick(CLICKABLE_POINT));
					...
					BOOL fFoundTimewarp = SUCCEEDED(LOG_FAILED_HR(WOWFromPartialQueryId(RESTORE_PREVIOUS_VERSIONS)));
					...
					if (GetProductInfo(WINDOWS_VISTA_VERSION, 0, 0, 0, &dwProductInfoType))
					{
						switch (dwProductInfoType)
						{
							...
							case PRODUCT_HOME_PREMIUM:
								LogTestInfoW(__FUNCTIONW__, L"Timewarp is not expected here!");
								break;
							...
							case PRODUCT_ULTIMATE:
								fExpected = TRUE;
								pwszResultInfo = L"Timewarp expected here.";
								break;
							default:
						...
						if (fFoundTimewarp != fExpected)
						{
							LogTestInfoW(__FUNCTIONW__,
								L"Timewarp SKU differentiation failed.[Current: %s | Expected: %s ]",
								fFoundTimewarp ? L"Found": L"Not Found",
								fExpected ? L"Found" : L"Not Found");
						}
						else
						{
							fPassed = TRUE;
							pwszResultInfo = L"Timewarp entry point is differentiated correctly";
							if (fExpected)
							{
    ...
    hr = LOG_FAILED_HR(WOWFromPartialQueryId(START_BUTTON));
    if (SUCCEEDED(hr))
    {
       LOG_FAILED_HR(LeftButtonClick(CLICKABLE_POINT));
    }
