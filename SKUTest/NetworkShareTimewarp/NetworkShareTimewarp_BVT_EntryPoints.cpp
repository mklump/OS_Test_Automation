...
STDAPI VerifyNetworkShareTimewarp(LPVOID pvReserved)
{
    BOOL    fPassed = FALSE;
    LPWSTR pwszResultInfo = NULL;
    HRESULT hr = E_FAIL;
    
	LogTestBeginW(__FUNCTIONW__, L"Verify Restore Previous Version available on Network Share...");    
	// Log WEI scores
	Assessment();
	...
	LOG_FAILED_HR(LeftButtonClick(CLICKABLE_POINT)); // added for Pseudo loc esp
	...
	if(FAILED(hr))
	{
		...
		if (fFoundRestorePreviousVersion!= fExpected)
		{
			...
			L" Network Share Timewrap SKU differentiation failed.[Current: %s | Expected: %s ]",
		...
		{
			fPassed = TRUE;
			pwszResultInfo = L" Network Share Timewrap entry point is differentiated correctly";
		}
	...
	ComplexResourceKeyNative cmplxKeyPropertiesFolder(1, resKeyPropertiesTitleBar, langNeutralTestFolder);

	hr = LOG_FAILED_HR(WOWFromPartialQueryId(FOLDER_PREVIOUS_VERSION_TAB));
	if(SUCCEEDED(hr))
	{
	...
	{
		LogTestInfoW(__FUNCTIONW__,
		L" Timewrap on Networkshare - SKU differentiation failed.[Current: %s | Expected: %s ]",
		fFoundRestorePreviousVersion ? L"Found": L"Not Found",
		fExpected ? L"Found" : L"Not Found");
	}
	else
	{
		fPassed = TRUE;
		pwszResultInfo = L" Timewrap on Networkshare (Previous Version Tab) entry point is differentiated correctly";
	}
